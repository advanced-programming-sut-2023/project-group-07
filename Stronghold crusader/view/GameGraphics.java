...
public class GameGraphics extends Application {
    ...
    public void start(Stage stage) {
        ...
        map = game.getMap();
        Scene scene = prepareScene(stage);
        addMoveTimeLine(stage, scene);
        handleSelectBuilding(stage);
        handleMouseDragged(stage);
        selectArea(stage);
//        setZoomOut(mapPane);
    ...
    private void selectArea(Stage stage) {
    ...
    private void handleMouseDragged(Stage stage) {
    ...
    private void handleSelectBuilding(Stage stage) {
    ...
    private void addMoveTimeLine(Stage stage, Scene scene) {
        Timeline timeline = new Timeline(new KeyFrame(Duration.millis(5), actionEvent -> {
            Robot robot = new Robot();
            if (robot.getMouseX() > scene.getWidth() - 50 && mapPane.getLayoutX() >/*-map.getSize()*40*/ -400*40) //TODO map is null
                mapPane.setLayoutX(mapPane.getLayoutX() - 10);
            if (robot.getMouseX() < 50 && mapPane.getLayoutX() < 0)
                mapPane.setLayoutX(mapPane.getLayoutX() + 10);
            if (robot.getMouseY() > scene.getHeight() - 10 && mapPane.getLayoutY() >/*-map.getSize()*40*/ -400*40)
                mapPane.setLayoutY(mapPane.getLayoutY() - 10);
            if (robot.getMouseY() < 50 && mapPane.getLayoutY() < 0)
                mapPane.setLayoutY(mapPane.getLayoutY() + 10);
            Rectangle miniMapRectangle = (Rectangle) miniMapPane.getChildren().get(miniMapPane.getChildren().size()-1);
            miniMapRectangle.setTranslateY(16+225*Math.abs(mapPane.getLayoutY())/(40*400)); //TODO change 400
            miniMapRectangle.setTranslateX(12.5+225*Math.abs(mapPane.getLayoutX())/(40*400));
        }));
    ...
    private Scene prepareScene(Stage stage) {
        ...
        mapPane.setMaxHeight(Main.screenHeight - 215);
        initStatusBar();
    ...
    private void showMap() {
        ...
        for (int i = 0 ; i < size; i++) {
            for (int j = 0 ; j < size ; j++) {
                mapPane.add(new ImageView(new Image(field.get(i).get(j).getTexture().getImagePath())), j, i);
            }
        }
    }
