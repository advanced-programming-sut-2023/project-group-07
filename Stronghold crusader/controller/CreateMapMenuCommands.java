...
public enum CreateMapMenuCommands {
    ...
    SET_REGION_TEXTURE("^\\s*settexture\\s+(\\-x1\\s+\\-?\\d+|\\-y1\\s+\\-?\\d+|\\-x2\\s+\\-?\\d+|\\-y2\\s+\\-?\\d+|\\-t\\s+[a-z\\s]+|\\s)+$"),
    ...
    DROP_UNIT("^\\s*dropunit\\s+(\\-x\\s+\\-?\\d+|\\-y\\s+\\-?\\d+|\\-t\\s+[a-z\\s]+|\\-c\\s+\\d+|\\-color\\s+[a-z]+|\\s)+$"),
    DROP_BUILDING ("^\\s*dropbuilding\\s+(\\-x\\s+\\-?\\d+|\\-y\\s+\\-?\\d+|\\-t\\s+[a-z\\s]+|\\-color\\s+[a-z]+|\\s)+$"),
