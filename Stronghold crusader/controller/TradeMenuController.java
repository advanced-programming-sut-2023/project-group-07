...
public class TradeMenuController {
    ...
    public Messages acceptTrade(int id, String message) {
        ...
        int price = targetRequest.price();
        if (requester.getGold() < price)
            return Messages.POOR_REQUESTER;
        int amount = targetRequest.amount();
        Resources resource = targetRequest.resource();
        if (receiver.getResourceAmount(resource) < amount)
            return Messages.POOR_RECEIVER;
    ...
    public Messages requestTrade(String typeName, String colorString, String message, int amount, int price) {
        if (price < 0)
            return Messages.NEGATIVE_PRICE;
        Resources resource = Resources.getResource(typeName);
        if (resource == null)
            return Messages.INVALID_RESOURCE;
